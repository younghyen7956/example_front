...
import { REQUEST_TO_TEXT_FASTAPI, SET_RAW_DATA,SET_USER_SESSION } from "./mutation-types";

export interface AnotherMutations extends MutationTree<AnotherState> {
  [REQUEST_TO_TEXT_FASTAPI](state: AnotherState, chunk: string): void;
...
const mutations: AnotherMutations = {
  ...
  [REQUEST_TO_TEXT_FASTAPI](state, chunk) {
    state.rawdata += chunk;
  },
